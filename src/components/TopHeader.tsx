  ...
  return (
    <header className="h-14 flex items-center justify-between px-6 border-b border-white/[0.06] bg-[#2D0A2D]/80 backdrop-blur-xl shrink-0">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-semibold text-[#4D1F4D] uppercase tracking-[0.1em] ml-1">
          Approval Dashboard
        </span>
      </div>
      <div className="flex items-center gap-3">
        <span className="text-xs text-white/40">
          Logged in as <span className="text-white/60 font-medium">{displayName}</span>
        </span>
        <button
          onClick={logout}
